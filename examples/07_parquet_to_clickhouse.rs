...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch = RecordBatch::try_new(schema.clone(), vec![
        ...
        Arc::new(
            Decimal128Array::from(vec![100050, 250075, 500025]).with_precision_and_scale(18, 2)?,
        ),
        Arc::new({
    ...
    ctx.register_parquet("parquet_data", parquet_path, Default::default()).await?;
    ...
    let _ = ctx
        .sql(&format!("INSERT INTO clickhouse.{}.users SELECT * FROM parquet_data", db))
        .await?
        .collect()
        .await?;
    ...
    println!("   All records:");
    let _ = ctx
        .sql(&format!("SELECT * FROM clickhouse.{}.users ORDER BY id", db))
        .await?
        .show()
        .await?;
    ...
    let _ = ctx
        .sql(&format!(
            "SELECT COUNT(*) as total, SUM(amount) as total_amount FROM clickhouse.{}.users",
            db
        ))
        .await?
        .show()
